...
- Autosave on committed change (not per-keystroke)
...
### 1) Database timestamp correctness
- Add new drizzle migration to normalize legacy seconds timestamps into ms
- Update remote action write expressions from second epoch to millisecond epoch

### 2) Backend actions for set metric persistence
- Add/extend workout remote form action(s) to update set fields:
  - `reps`
  - `weight`
  - `duration`
- Validate by exercise measurement mode and DB constraints
- Ensure mutation is scoped to the current workout + set

### 3) Completion validation on server
- Update `toggleSetComplete` so marking complete requires metric fields based on exercise type
- Keep marking incomplete permissive
- Return actionable form issues for inline UI rendering

### 4) Workout page timer state + rendering
- Add sticky/global timer region with:
  - wall-clock (`HH:MM:SS`)
  - rest countdown
- Use client tick loop (1-second cadence) with cleanup on destroy
- Reconstruct rest timer from latest completed set on load
...
- Autosave each field on committed change
